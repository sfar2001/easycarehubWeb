/* =====================================================================
   Claude tool-use loop. Given one piece of Userback feedback, Claude
   can read/list/write files under an allow-list and then call `done`
   to finish. File-system access is sandboxed to the repo root and to
   a whitelist of source paths — no infra files may be touched.
   ===================================================================== */
...
const ALLOW_READ_EXTRA = [
  /^README\.md$/,
  /^netlify\.toml$/, // read-only, for context
  /^assets\/css\//,  // css is writable anyway, listed here for clarity
];
...
const TOOLS = [
  {
    name: "list_files",
    description:
      "List files under a directory (relative to repo root). Returns only files the bot is allowed to read. Max 500 entries.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "write_file",
    description:
      "Overwrite a file with new contents. MUST be an allowed path; otherwise the call fails. Preserve trailing newline if the original has one.",
    input_schema: {
      type: "object",
      properties: {
        path:    { type: "string" },
...
You receive ONE piece of user feedback from Userback. Decide whether the feedback can be addressed with a small code change to the site, and if so, make it.

Workflow:
1. If the feedback is a compliment, a question, a hardware/server issue, unintelligible, or otherwise NOT actionable as a website code change, call \`done\` with should_open_pr=false and explain briefly.
...
function toolListFiles(root, { dir = "." } = {}) {
  ...
  if (entries.length === 0) return "(no readable files)";
  return entries.join("\n");
}
...
export async function analyseAndFix(feedback, repoRoot) {
  ...
  const messages = [
    {
      role: "user",
      content: [{ type: "text", text: buildUserPrompt(feedback) }],
    },
  ];
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    if (resp.stop_reason !== "tool_use") {
      // Plain text response — treat as implicit done with no changes
    ...
    for (const block of resp.content) {
      ...
      try {
        ...
        switch (block.name) {
          case "list_files":
            content = toolListFiles(repoRoot, block.input);
            break;
          case "read_file":
            content = toolReadFile(repoRoot, block.input);
            break;
          case "write_file":
...
function buildUserPrompt(fb) {
  const meta = [
    `Feedback ID: ${fb.id}`,
    fb.title ? `Title: ${fb.title}` : null,
    fb.category ? `Category: ${fb.category}` : null,
    fb.priority ? `Priority: ${fb.priority}` : null,
    fb.rating !== undefined ? `Rating: ${fb.rating}` : null,
    fb.name || fb.email ? `Reporter: ${fb.name || ""} ${fb.email ? `<${fb.email}>` : ""}`.trim() : "Reporter: anonymous",
    fb.url ? `Page URL: ${fb.url}` : null,
    fb.browser_name ? `Browser: ${fb.browser_name}` : null,
    fb.os_name ? `OS: ${fb.os_name}` : null,
    `Submitted: ${fb.created_at}`,
  ]
    .filter(Boolean)
    .join("\n");

  return `## User feedback to evaluate
